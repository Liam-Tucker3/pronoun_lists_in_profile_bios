...
def get_tokens_data():
    ...
    keywords = ['binary', 'nonbinary', 'non-binary', 'non binary', 'multiple_nonbinary_signals']
    list_of_dicts = []
    for key in keywords:
        this_dict = {}
        this_dict['token'] = key
    ...
    for row in getBioCursor:
        ...
        for i in add_list:
            ...
            if (this_bio_pronouns != 'none'):
                this_dict['pronoun list'] += 1
                this_dict[this_bio_pronouns] += 1
...
def convert_to_prevalences(list_of_dicts2, total_counts2, ov_cutoff, pr_cutoff):
    ...
    for i in range(len(prev_frame)):
        ...
        for pronoun in columns[1:]:
            ...
            else : temp_dict[pronoun] = round(1.0 * temp_dict[pronoun] / total_counts2[pronoun], 3)
    ...
    # Sorting list_of_dicts
    prev_frame.insert(0, total_counts2)
    prev_frame = sorted(prev_frame, key = lambda x:(x['pronoun list'], x['overall']), reverse=True)
    
    # Creating final csv file  
    token_file_name = output_dir + 'pronouns_binary_prob.csv'
    with open(token_file_name, 'w', encoding="utf-8") as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames = columns)
        writer.writeheader()
